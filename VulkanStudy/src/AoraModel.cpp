...
namespace aor
{
	AoraModel::AoraModel(AoraDevice& device, const std::vector<Vertex>& vertices)
		: aoraDevice{device}
	{
	...
	void AoraModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		void* data;
		vkMapMemory(aoraDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(aoraDevice.device(), vertexBufferMemory);
	}
