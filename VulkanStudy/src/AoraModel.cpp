...
namespace aor
{
	...
	void AoraModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		vertexBuffer = std::make_unique<AoraBuffer>(
			aoraDevice,
			vertexSize,
			vertexCount,
		...
		aoraDevice.copyBuffer(stagingBuffer.getBuffer(), vertexBuffer->getBuffer(), bufferSize);
	}
	...
	void AoraModel::createIndexBuffers(const std::vector<uint32_t>& indices)
	{
		...
		indexBuffer = std::make_unique<AoraBuffer>(
			aoraDevice,
			indexSize,
			indexCount,
		...
		aoraDevice.copyBuffer(stagingBuffer.getBuffer(), indexBuffer->getBuffer(), bufferSize);
	}
